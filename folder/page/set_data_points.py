...
data_y=numpy.random.random((1,len(tk)))
...
trend_x=numpy.random.random((1,len(tk)))
trend_x_vals=list(trend_x[0])
trend_y_vals=list(trend_y)

with open("token_trend_x.json","w") as w_file:
    json.dump(trend_x_vals,w_file)

with open("token_trend_y.json","w") as w_file:
    json.dump(trend_y_vals,w_file)
